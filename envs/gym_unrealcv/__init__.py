...
#track
for env in ['TrackRoom']:# , 'UrbanCity', 'UrbanRoad', 'Garage', 'SnowForest', 'Forest', 'Garden']:
...
# # UnrealCvGeometryAD
# for env in ['GeometryTrackAD']:# , 'UrbanCity', 'UrbanRoad', 'Garage', 'SnowForest', 'Forest', 'Garden']:
#     setting_file = 'track/{env}.json'.format(env=env)
#     settings = load_env_setting(setting_file)
#     for i, reset in enumerate(['random', 'waypoint', 'testpoint']):  # reset type
#         for action in ['Discrete', 'Continuous']:  # action type
...
#                     register(
#                         id=name,
#                         entry_point='gym_unrealcv.envs:UnrealCvGeometryAD',
#                         kwargs={'setting_file': setting_file,
#                                 'category': category,
#                                 'reset_type': 0,
#                                 'action_type': action,
...
for env in ['GlassRoom']:# , 'UrbanCity', 'UrbanRoad', 'Garage', 'SnowForest', 'Forest', 'Garden']:
    setting_file = 'glass/{env}.json'.format(env=env)
    settings = load_env_setting(setting_file)
    for i, reset in enumerate(['random', 'waypoint', 'testpoint']):  # reset type
        for action in ['Discrete', 'Continuous']:  # action type
            for obs in ['Color', 'Depth', 'Rgbd']:  # observation type
                for category in settings['targets']:
                    name = 'UnrealGlass-{env}{category}-{action}{obs}-v{reset}'.format(
                        env=env, category=category, action=action, obs=obs, reset=i)
                    setting_file = 'glass/{env}.json'.format(env=env)
                    register(
                        id=name,
                        entry_point='gym_unrealcv.envs:UnrealCvGlass',
                        kwargs={'setting_file': setting_file,
                                'category': category,
                                'reset_type': reset,
                                'action_type': action,
...
for env in ['GlassRoom']:# , 'UrbanCity', 'UrbanRoad', 'Garage', 'SnowForest', 'Forest', 'Garden']:
    ...
    for i, reset in enumerate(['random', 'waypoint', 'testpoint']):  # reset type
        for action in ['Discrete', 'Continuous']:  # action type
            for obs in ['Color', 'Depth', 'Rgbd']:  # observation type
                for category in settings['targets']:
                    name = 'UnrealGlassLocate-{env}{category}-{action}{obs}-v{reset}'.format(
                        env=env, category=category, action=action, obs=obs, reset=i)
                    setting_file = 'glass/{env}.json'.format(env=env)
                    register(
                        id=name,
                        entry_point='gym_unrealcv.envs:UnrealCvGlassLocate',
                        kwargs={'setting_file': setting_file,
                                'category': category,
                                'reset_type': reset,
                                'action_type': action,
